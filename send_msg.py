...
#     url = f'https://api.telegram.org/bot{env['_telegram_token']}/sendMessage'
#
...
if __name__ == '__main__':
    ...
    async def main():
        env: Dict[str: str] = Config().get_config(ConfigNames.TELEGRAM)
        # Использование:
        # messenger_1 = TelegramMessenger()
        ...
        _msg_copy_5 = 100
        for idx in range(50):
            await TelegramMessenger().add_message(
                f'Сообщение 1:\n'
                f'{f"{idx + 1}, " * _msg_copy_1}')
            await TelegramMessenger().add_message(
                f'Сообщение 2:\n'
                f'{f"{idx + 1}, " * _msg_copy_2}')
            await TelegramMessenger().add_message(
                f'Сообщение 3:\n'
                f'{f"{idx + 1}, " * _msg_copy_3}')
            await TelegramMessenger().add_message(
                f'Сообщение 4:\n'
                f'{f"{idx + 1}, " * _msg_copy_4}')
            await TelegramMessenger().add_message(
                f'Сообщение 5:\n'
                f'{f"{idx + 1}, " * _msg_copy_5}')
